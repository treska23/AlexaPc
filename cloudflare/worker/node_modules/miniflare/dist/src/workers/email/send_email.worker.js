...
function captureRawForJsonRow(metadata, raw, includeTruncationMarker = !1) {
  assertMetadataFits(metadata);
  let initial = captureRawForJsonRowMetadata(metadata, raw);
  return !initial.truncated || !includeTruncationMarker ? initial : captureRawForJsonRowMetadata(
    { ...metadata, captureTruncated: !0 },
    raw
  );
}
function captureRawForJsonRowMetadata(metadata, raw) {
  assertMetadataFits(metadata);
  let emptyRecord = { ...metadata, rawBase64: "" }, emptyRecordBytes = jsonByteLength(emptyRecord);
  if (emptyRecordBytes > 1998976)
    return {
      email: { ...metadata },
      truncated: raw.byteLength > 0
    };
  let availableBase64Bytes = 1998976 - emptyRecordBytes, captured = captureRawForBase64Budget(raw, availableBase64Bytes);
  return {
    email: { ...metadata, rawBase64: captured.rawBase64 },
    truncated: captured.truncated
  };
}
function captureTextAndHtmlForJsonRow(metadata, text, html, includeTruncationMarker = !1) {
  let initial = captureTextAndHtmlForJsonRowMetadata(metadata, text, html);
  return !initial.truncated || !includeTruncationMarker ? initial : captureTextAndHtmlForJsonRowMetadata(
    { ...metadata, captureTruncated: !0 },
    text,
    html
  );
}
function captureTextAndHtmlForJsonRowMetadata(metadata, text, html) {
  assertMetadataFits(metadata);
  let email = { ...metadata }, truncated = !1;
  if (text !== void 0) {
    let captured = captureOptionalStringField(email, "text", text);
    email = captured.email, truncated ||= captured.truncated;
  }
  if (html !== void 0) {
    let captured = captureOptionalStringField(email, "html", html);
    email = captured.email, truncated ||= captured.truncated;
  }
  return { email, truncated };
}
function assertMetadataFits(metadata) {
  if (jsonByteLength(metadata) > 1998976)
    throw new RangeError("Email metadata exceeds the 2 MB storage row limit");
}
function captureRawForBase64Budget(raw, maxBase64Bytes) {
  let maxRawBytes = Math.max(0, Math.floor(maxBase64Bytes / 4)) * 3, truncated = raw.byteLength > maxRawBytes, captured = truncated ? raw.subarray(0, maxRawBytes) : raw;
  return { rawBase64: bytesToBase64(captured), truncated };
}
function captureOptionalStringField(email, field, value) {
  let emptyEmail = { ...email, [field]: "" };
  return jsonByteLength(emptyEmail) > 1998976 ? { email, truncated: value.length > 0 } : captureStringField(emptyEmail, field, value);
}
function captureStringField(email, field, value) {
  let fullEmail = { ...email, [field]: value };
  if (jsonByteLength(fullEmail) <= 1998976)
    return { email: fullEmail, truncated: !1 };
  let lower = 0, upper = value.length;
  for (; lower < upper; ) {
    let middle = Math.ceil((lower + upper) / 2), candidate = safeStringPrefix(value, middle), candidateEmail = { ...email, [field]: candidate };
    jsonByteLength(candidateEmail) <= 1998976 ? lower = middle : upper = middle - 1;
  }
  return {
...
function safeStringPrefix(value, length) {
  ...
  return end > 0 && end < value.length && value.charCodeAt(end - 1) >= 55296 && value.charCodeAt(end - 1) <= 56319 && value.charCodeAt(end) >= 56320 && value.charCodeAt(end) <= 57343 && end--, value.slice(0, end);
}
...
// src/workers/email/send_email.worker.ts
$.enabled = !0;
var MAX_ATTACHMENT_EXTENSION_LENGTH = 32;
function getAttachmentExtension(filename) {
  let extension = filename.match(/\.([^.]+)$/u)?.[1];
  return extension !== void 0 && extension.length <= MAX_ATTACHMENT_EXTENSION_LENGTH && /^[A-Za-z0-9][A-Za-z0-9._-]*$/u.test(extension) ? extension : "bin";
}
...
var SendEmailBinding = class extends WorkerEntrypoint {
   ...
   */
  async log(message, level = LogLevel2.INFO) {
   ...
   * `id` names the file.
   */
  async storeTempFile(content, extension, prefix, id) {
    let resp = await storeEmailTempFile(
  ...
  checkDestinationAllowed(to) {
    if (this.env.destinationAddress !== void 0 && to !== this.env.destinationAddress)
      throw new Error(`email to ${to} not allowed`);
    if (this.env.allowedDestinationAddresses !== void 0 && !this.env.allowedDestinationAddresses.includes(to))
      throw new Error(`email to ${to} not allowed`);
  }
  checkSenderAllowed(from) {
    if (this.env.allowedSenderAddresses !== void 0 && !this.env.allowedSenderAddresses.includes(from))
  ...
  async send(emailMessageOrBuilder) {
    if (this.isEmailMessage(emailMessageOrBuilder)) {
      ...
      let messageId = synthesizeMessageId(emailMessage.from), id = messageIdToStorageId(messageId), normalizedRawEmailBuffer = setMessageIdHeader(
        rawEmailBuffer,
        messageId
      ), normalizedParsedEmail = await PostalMime.parse(
        normalizedRawEmailBuffer
      );
      return await this.captureSentEmail(
...
Email: ${filePath}`
      ...
      let toDisplay2 = function(addr) {
        return (Array.isArray(addr) ? addr : [addr]).map(formatEmailAddress);
      };
      var toDisplay = toDisplay2;
      let builder = emailMessageOrBuilder;
      this.validateMessageBuilder(builder);
      let messageId = synthesizeMessageId(extractEmailAddress(builder.from)), id = messageIdToStorageId(messageId), sentAttachments = (builder.attachments ?? []).map((attachment) => ({
      ...
      return await this.captureSentEmail(
        () => captureTextAndHtmlForJsonRow(
          {
            ...
            to: toDisplay2(builder.to),
            cc: builder.cc ? toDisplay2(builder.cc) : void 0,
            bcc: builder.bcc ? toDisplay2(builder.bcc) : void 0,
            replyTo: builder.replyTo ? formatEmailAddress(builder.replyTo) : void 0,
        ...
        (async () => {
          let files = [];
          if (builder.text) {
            let textPath = await this.storeTempFile(
              builder.text,
              "txt",
              "email-text",
              id
          ...
          if (builder.html) {
            let htmlPath = await this.storeTempFile(
              builder.html,
              "html",
              "email-html",
              id
          ...
          if (builder.attachments)
            for (let [index, attachment] of builder.attachments.entries()) {
              let extension = getAttachmentExtension(attachment.filename), attachmentPath = await this.storeTempFile(
                attachment.content,
                extension,
                "email-attachment",
                `${id}-${index + 1}`
