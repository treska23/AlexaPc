...
import { HttpError, LogLevel as LogLevel3, SharedHeaders as SharedHeaders2 } from "miniflare:shared";
...
function decodeErrorPayload(response) {
  let payload = response.headers.get(CoreHeaders.ERROR_STACK_PAYLOAD);
  if (payload === null)
    return null;
  try {
...
function stripEmailHeader(raw, headerName) {
  let removals = [], offset = 0, headerStart = 0, removeHeader = !1;
  for (; offset < raw.byteLength; ) {
    let line = findHeaderLine(raw, offset);
    if (line.contentEnd === offset) {
      removeHeader && (removals.push({ start: headerStart, end: offset }), removeHeader = !1);
      break;
    }
    raw[offset] === 32 || raw[offset] === 9 || (removeHeader && removals.push({ start: headerStart, end: offset }), headerStart = offset, removeHeader = headerNameMatches(
      raw,
      offset,
      line.contentEnd,
      headerName
  ...
  let removedBytes = removals.reduce(
    ...
    0
  ), stripped = new Uint8Array(raw.byteLength - removedBytes), sourceOffset = 0, targetOffset = 0;
  for (let removal of removals) {
    let retained = raw.subarray(sourceOffset, removal.start);
    stripped.set(retained, targetOffset), targetOffset += retained.byteLength, sourceOffset = removal.end;
  }
  return stripped.set(raw.subarray(sourceOffset), targetOffset), stripped;
}
function captureRawForBodyRow(raw) {
  return captureRawForBase64Budget(raw, 1998976);
}
...
function findHeaderLine(raw, start) {
  for (let index = start; index < raw.byteLength; index++) {
    if (raw[index] !== 10)
      continue;
    return { contentEnd: index > start && raw[index - 1] === 13 ? index - 1 : index, end: index + 1 };
  }
  return { contentEnd: raw.byteLength, end: raw.byteLength };
}
function headerNameMatches(raw, start, end, headerName) {
  let colon = start;
  for (; colon < end && raw[colon] !== 58; )
    colon++;
  if (colon === end || colon - start !== headerName.length)
    return !1;
  for (let index = 0; index < headerName.length; index++) {
    let byte = raw[start + index], lowerByte = byte >= 65 && byte <= 90 ? byte + 32 : byte, expected = headerName.charCodeAt(index), lowerExpected = expected >= 65 && expected <= 90 ? expected + 32 : expected;
...
function buildReplyFromMessageBuilder(builder, incomingMessage, recipient) {
  ...
  let messageId = synthesizeMessageId(extractEmailAddress(builder.from)), references = incomingMessage.references === void 0 ? incomingMessageId : `${incomingMessage.references} ${incomingMessageId}`, raw = buildMimeMessage(
    {
      from: sender,
      to: [recipient],
      replyTo,
...
async function handleEmail(params, request, service, workerName, env, ctx) {
  ...
  let store = env[CoreBindings.SERVICE_EMAIL_STORE], storedFrom = from, storedTo = to, receivedAt = (/* @__PURE__ */ new Date()).toISOString(), stored = !1;
...
function getUserRequest(request, env, clientIp) {
  ...
  if (proxySharedSecret) {
    let secretFromHeader = encoder2.encode(proxySharedSecret), configuredSecret = env[CoreBindings.DATA_PROXY_SHARED_SECRET];
...
function getTargetService(request, url, env) {
  let override = request.headers.get(CoreHeaders.ROUTE_OVERRIDE);
...
function maybePrettifyError(request, response, env) {
  if (response.status !== 500 || response.headers.get(CoreHeaders.ERROR_STACK) === null)
    return response;
  let body = response.body ?? decodeErrorPayload(response);
  return body === null ? response : env[CoreBindings.SERVICE_LOOPBACK].fetch(
...
function maybeLogRequest(req, res, env, ctx, startTime) {
  ...
  if (res.headers.delete(ADDITIONAL_RESPONSE_LOG_HEADER_NAME), env[CoreBindings.JSON_LOG_LEVEL] < LogLevel3.INFO) return res;
  ...
  return ctx.waitUntil(
    env[CoreBindings.SERVICE_LOOPBACK].fetch("http://localhost/core/log", {
      method: "POST",
      headers: { [SharedHeaders2.LOG_LEVEL]: LogLevel3.INFO.toString() },
      body: message
    })
  ), res;
}
...
var entry_worker_default = {
  async fetch(request, env, ctx) {
    ...
    request = new Request(request, { cf }), request.headers.delete(CoreHeaders.ACCESS_BLOB);
    ...
    let url = new URL(request.url), { service, routeTarget } = getTargetService(request, url, env);
    if (service === void 0)
    ...
    try {
      ...
      if ((url.pathname === CorePaths.IMAGE_DELIVERY || url.pathname.startsWith(`${CorePaths.IMAGE_DELIVERY}/`) || url.pathname === CorePaths.IMAGE_UPLOAD || url.pathname.startsWith(`${CorePaths.IMAGE_UPLOAD}/`)) && imagesDelivery)
      ...
      let r2S3Service = env[CoreBindings.SERVICE_R2_S3];
      if ((url.pathname === CorePaths.R2_S3 || url.pathname.startsWith(`${CorePaths.R2_S3}/`)) && r2S3Service) {
        let s3Request = request, originalHostname = request.headers.get(
          CoreHeaders.ORIGINAL_HOSTNAME
        );
        if (originalHostname !== null) {
          let s3Url = new URL(url);
          s3Url.host = originalHostname, s3Request = new Request(s3Url, request), s3Request.headers.set("Host", originalHostname);
        }
        return await r2S3Service.fetch(s3Request);
      }
      let accessBlob = env[`${CoreBindings.JSON_ACCESS_BLOB_PREFIX}${routeTarget}`];
      if (accessBlob) {
        let headers = new Headers(request.headers);
        headers.set(CoreHeaders.ACCESS_BLOB, JSON.stringify(accessBlob)), request = new Request(request, { headers });
      }
      let response = await service.fetch(request);
      return disablePrettyErrorPage || (response = await maybePrettifyError(request, response, env)), response = ensureAcceptableEncoding(clientAcceptEncoding, response), env[CoreBindings.LOG_REQUESTS] && (response = maybeLogRequest(request, response, env, ctx, startTime)), response;
    } catch (e) {
